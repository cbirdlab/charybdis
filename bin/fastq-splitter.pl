...
my ($opt_n_parts,$opt_part_size,$opt_measure,$opt_outdir,$opt_eol,$opt_check,$opt_version,$opt_help);
...
Options:
    ...
    --outdir <path>      - Choose output directory (same as input file by default).
...
my $outdir = defined($opt_outdir) ? $opt_outdir : '-1';
...
sub split_file
{
    ...
    if ($part_size)
    {
        ...
        {
           $based = basename($base);
           $out_file = sprintf("%s/%s.part-%d%s",$outdir,$based,$part,$ext);
        }
        else
        {
           $out_file = sprintf("%s.part-%d%s",$base,$part,$ext);
        }
    ...
    else
    {
        ...
        for (my $part = 1; $part <= $n_parts; $part++)
        {
            ...
            $part_file .= sprintf("-%d%s",$part,$ext);
